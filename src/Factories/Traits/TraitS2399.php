...
trait TraitS2399
{
    ...
    protected function toNodeS100()
    {
        ...
        $this->dom->addChild(
            $infoTSVTermino,
            "percAliment",
            !empty($this->std->infotsvtermino->percaliment) ? $this->std->infotsvtermino->percaliment : null,
            false
        );
        $this->dom->addChild(
            $infoTSVTermino,
            "vrAlim",
            !empty($this->std->infotsvtermino->vralim) ? $this->std->infotsvtermino->vralim : null,
            false
        );
        $this->dom->addChild(
            $infoTSVTermino,
            "nrProcTrab",
            !empty($this->std->infotsvtermino->nrproctrab) ? $this->std->infotsvtermino->nrproctrab : null,
            false
        );
        ...
        if (!empty($this->std->infotsvtermino->verbasresc)) {
            ...
            if (!empty($this->std->infotsvtermino->verbasresc->procjudtrab)) {
                foreach ($this->std->infotsvtermino->verbasresc->procjudtrab as $pj) {
                    $procJudTrab = $this->dom->createElement("procJudTrab");
                    $this->dom->addChild(
                        $procJudTrab,
                        "tpTrib",
